...
export VMESS_ARGO=${VMESS_ARGO:-'11226'}
export VMESS_TLS=${VMESS_TLS:-'11227'}
...
ingress:
  - hostname: $ARGO_DOMAIN
    service: http://localhost:$VMESS_ARGO
    originRequest:
      noTLSVerify: true
  ...
  cat > config.json << EOF
{
  ...
  "dns": {
    ...
    },
    {
      ...
      "type": "vmess",
      "listen": "::",
      "listen_port": $VMESS_TLS,  // 端口号，通常使用 443
      "users": [
        {
          "uuid": "$UUID"  // 新的 UUID
        }
      ...
      "transport": {
        ...
        "tls": {
          ...
          "certificates": [
            {
              "certificate_file": "/home/$USERNAME/serv00_cert.pem",  // TLS 证书路径
              "key_file": "/home/$USERNAME/serv00_key.pem"  // TLS 密钥路径
            }
          ]
        }
      }
...
if [ -e "$(basename ${FILE_MAP[bot]})" ]; then
    if [[ $ARGO_AUTH =~ ^[A-Z0-9a-z=]{120,250}$ ]]; then
      ...
      args="tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile boot.log --loglevel info --url http://localhost:$VMESS_ARGO"
    fi
...
cat > list.txt <<EOF
vmess://$(echo "{ \"v\": \"2\", \"ps\": \"$NAME-vmss\", \"add\": \"$IP\", \"port\": \"$VMESS_TLS\", \"id\": \"$UUID\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"\", \"path\": \"/vmess-argo?ed=2048\", \"tls\": \"\", \"sni\": \"\", \"alpn\": \"\", \"fp\": \"\"}" | base64 -w0)
